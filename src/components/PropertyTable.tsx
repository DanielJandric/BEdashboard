...
const formatPercentage = (value: number | null, decimals = 2): string => {
  ...
  const displayValue =
    numValue <= 1 && numValue >= 0 ? numValue * 100 : numValue;
...
const ALL_COLUMNS: ColumnConfig[] = [
  ...
  {
    key: "valeurcbrechf",
    header: "Valeur CBRE (CHF)",
    sortable: true,
    format: formatCurrency,
    numeric: true,
  },
  {
    key: "loyerannuelsourcechf",
    header: "Loyer annuel source (CHF)",
    sortable: true,
    format: formatCurrency,
    numeric: true,
  },
  {
    key: "loyerfutursourcechf",
    header: "Loyer futur source (CHF)",
    sortable: true,
    format: formatCurrency,
    numeric: true,
  },
  {
    key: "rendbrutsource",
    header: "Rend. brut source %",
    sortable: true,
    format: formatPercentage,
    numeric: true,
  },
  {
    key: "constructionrnovation",
    header: "Constr./Rénovation",
    sortable: true,
  },
  {
    key: "surfacelocm",
    header: "Surface loc. (m²)",
    sortable: true,
    format: formatNumber,
    numeric: true,
  },
  ...
  {
    key: "loyeractuelchfancbre",
    header: "Loyer actuel (CHF/an)",
    sortable: true,
    format: formatCurrency,
    numeric: true,
  },
  {
    key: "loyerpotentielchfancbre",
    header: "Loyer potentiel (CHF/an)",
    sortable: true,
    format: formatCurrency,
    numeric: true,
  },
  ...
  { key: "toiture", header: "Toiture", sortable: true },
  { key: "fentres", header: "Fenêtres", sortable: true },
  {
    key: "chauffagetypeanne",
    header: "Chauffage (type/année)",
    sortable: true,
  },
  { key: "faade", header: "Façade", sortable: true },
  ...
  {
    key: "tauxdescomptenominal",
    header: "Taux escompte nominal",
    sortable: true,
    format: formatPercentage,
    numeric: true,
  },
  {
    key: "tauxcapitalisationexit",
    header: "Taux cap. exit",
    sortable: true,
    format: formatPercentage,
    numeric: true,
  },
...
const PropertyTable: React.FC<PropertyTableProps> = ({
  ...
  const columns = useMemo(() => {
    ...
    const currentCols: ColumnConfig[] = visibleColumns.has("actions")
      ? [...dataCols, actionsCol]
      : dataCols;
    return currentCols;
  }, [visibleColumns]);
  ...
  const isAllVisibleSelected = useMemo(() => {
    return (
      ...
      sortedProperties.every((prop) => selectedProperties.has(prop.id))
    );
  ...
  const masterCheckboxState = isAllVisibleSelected
    ? true
      ...
      <div className="mb-4 flex justify-end">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              className="ml-auto bg-white border-be_capital_grey text-be_capital_dark_grey hover:bg-gray-50"
            >
              Columns
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuSeparator />
            {[...ALL_COLUMNS, { key: "actions", header: "Actions" }].map(
              (column) => (
                <DropdownMenuCheckboxItem
                  key={column.key}
            ...
            <tr>
              ...
              {columns.map((col) => (
                <th
                  key={col.key}
                  scope="col"
                  className={`px-4 py-3 text-left text-xs font-medium text-be_capital_dark_grey uppercase tracking-wider ${
            ...
            {sortedProperties.map((prop) => (
              <tr
                ...
                className={`hover:bg-gray-50 ${
                  selectedProperties.has(prop.id) ? "bg-yellow-50" : ""
                }`}
              >
